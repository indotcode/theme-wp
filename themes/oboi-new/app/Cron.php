...
//    $schedules['my_cron_category'] = array( 'interval' => 60 * 60, 'display' => 'Импорт категорий' );
    ...
    $schedules['my_cron_goods_images'] = array( 'interval' => 60 * 60 * 10 , 'display' => 'Конвертация картинок каталога' );
    return $schedules;
}
//
...
if ( ! wp_next_scheduled( 'my_cron_worker_goods_images' ) ) {
    wp_schedule_event( time(), 'my_cron_goods_images', 'my_cron_worker_goods_images' );
}
//
...
add_action( 'my_cron_worker_goods_images', 'import_goods_images_function' );
function import_goods_images_function() {
    Images::ResizeImagesFullConvert();
}
