    ...
    <main>
        <section class="shop">
            <div class="shop__wrapper wrapper">
                <div class="shop__field shop__field--full">
                    <div class="shop__breadcrumbs breadcrumbs">
                        <div class="breadcrumbs__wrapper wrapper">
                            ...
                            <a class="breadcrumbs__link" href="/search-goods/<?=get_query_var('search')?>">
                                Поиск "<?=get_query_var('search')?>"
                            </a>
                        </div>
                    </div>
                        ...
                        <div class="shop__setting-sort">
                            <form class="shop__setting-choose ajax-jq" action="/wp-ajax/index.php" method="post">
                                ...
                                <div class="shop__setting-container-value">
                                    <button type="submit" value="price_desc">По убыванию цены</button>
                                    <button type="submit" value="price_asc">По возрастанию цены</button>
                                    <button type="submit" value="title">По наименованию</button>
                                </div>
                            </form>
                        </div>
                        <form class="shop__setting-grid ajax" action="/wp-ajax/index.php" method="post">
                            <input type="hidden" name="type" value="grid">
                            <button type="submit" value="row" id="rowGrid" class="<?=$goods['grid'] == 'row' ? 'shop__setting-grid-active' : ''?>">
                                <svg width="24" height="18" viewBox="0 0 24 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <rect width="24" height="7.5" fill="#BDBDBD"/>
                                    <rect y="10.5" width="24" height="7.5" fill="#BDBDBD"/>
                                </svg>
                            </button>
                            <button type="submit" value="column" id="columnGrid" class="<?=$goods['grid'] == 'column' ? 'shop__setting-grid-active' : ''?>">
                            ...
                            <a href="<?=$item['guid']?>" class="shop__card card">
                                <div class="card__banner">
                                    <?php if(count($item['meta']['Фотографии']) != 0){ ?>
                                        <img class="card__img" src="<?=Ceo::$domain?><?=$item['meta']['Фотографии'][0]?>" alt="">
                                    <?php } else { ?>
