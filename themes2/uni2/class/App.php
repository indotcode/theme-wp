...
//        }
    ...
    public static function IconInfoBlock()
    {
        $result = [];
        for($i = 1; $i <= 13; $i++){
            $result[$i] = esc_url(get_template_directory_uri()) . '/assets/img/u' . $i . '.png';
        }
        return $result;
    }
    ...
    public static function ServicesHome($params = [])
    {
        $qu_result = [];
        $qu_result['taxonomy'] = 'cat-services';
        if(isset($params['include'])){
            $qu_result['include'] = $params['include'];
        }
        $qu_result['include'] = 'cat-services';
        $terms = get_terms($qu_result);
        $result = [];
        foreach ($terms as $key => $val){
            $post_home = get_term_meta($val->term_id, 'home-post', true);
            $val->home_post = $post_home['ID'];
            $val->post = (new WP_Query)->query([
                'post_type'   => 'services',
                'tax_query' => array(
                    array(
                        'taxonomy' => 'cat-services',
    ...
    public static function social($style)
    {
        $resultSoc = ['facebook', 'vk', 'instagram', 'behance'];
        $resultSocA = [];
        foreach ($resultSoc as $key => $val){
        $meta = get_post_meta(487, $val, true);
            if($meta != ''){
                $resultSocA[$val] = get_post_meta(487, $val, true);
            }
        }
        ?>
        <div class="social">
            <? foreach($resultSocA as $key => $val) : ?>
                <a href="<?=$val?>">
                    <img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/img/<?=$key?>-<?=$style?>.png" alt="">
                </a>
            <? endforeach; ?>
        </div>
        <?php
    ...
    public static function list_term_post($id_post)
    {
        $term_id = wp_get_post_terms($id_post, 'cat-services')[0]->term_id;
        $resultQuery = (new WP_Query())->query([
            'post_type' => 'services',
            'tax_query' => array(
                array(
                    'taxonomy' => 'cat-services',
